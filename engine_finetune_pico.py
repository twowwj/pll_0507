...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    args=None, data_loader_val=None, confidence=None, start_upd_prot=False):
	model.train(True)
	...
	for data_iter_step, (samples, labelset, targets, index) in enumerate(
		...
		with torch.cuda.amp.autocast():
			outputs, score_prot = model(samples, labelset)
			loss = criterion(outputs, index)

		if start_upd_prot:
			# print('start update epoch = {}'.format(args.start_update))
			criterion.confidence_update(temp_un_conf=score_prot, batch_index=index, batchY=labelset)
...
def evaluate(data_loader, model, device, logger):
	...
	logger.info('* Acc@1 {top1.global_avg:.3f} Acc@5 {top5.global_avg:.3f} loss {losses.global_avg:.3f}'
	            .format(top1=metric_logger.acc1, top2=metric_logger.acc2, top5=metric_logger.acc5, losses=metric_logger.loss))
