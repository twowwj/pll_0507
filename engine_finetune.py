...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    args=None, data_loader_val=None):
	model.train(True)
	...
	one_epoch_best = 0
	for data_iter_step, (samples, targets, idx) in enumerate(
		...
		# temp = loss_all.cpu().detach().numpy().tolist()
		# loss_all_list = [float('{:.4f}'.format(i)) for i in temp]
		# idx_list = idx.cpu().detach().numpy().tolist()
		# idx_loss = dict(zip(idx_list, loss_all_list))
		# if epoch >=70:
		# 	with open("/data/wjwang/pll_0507/rafdb/base_rafbd.txt", "a") as f:
		# 		f.write("{} \n ".format(idx_loss))
		# 		f.write("***" * 30)
		# 		f.write("current epoch {}".format(epoch))
		# 		f.write("***" * 30)
		# 		f.write("\n")
